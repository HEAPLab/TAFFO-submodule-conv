...
--------------------------------------------------------------------*/
  ...
  for (int ii=0; ii<5; ii++) {
    flo_rsdnm[ii] = rsdnm[ii];
    flo_errnm[ii] = errnm[ii];
  }
  verify ( flo_rsdnm, flo_errnm, flo_frc, &class, &verified );
...
static void blts (int nx, int ny, int nz, int k,
                  double FIXP_ANN omega,
/*--------------------------------------------------------------------
c   To improve cache performance, second two dimensions padded by 1 
c   for even number sizes only.  Only needed in v.
--------------------------------------------------------------------*/
                  double FIXP_ANN v[ISIZ1][ISIZ2/2*2+1][ISIZ3/2*2+1][5],
                  double FIXP_ANN ldz[ISIZ1][ISIZ2][5][5],
                  double FIXP_ANN ldy[ISIZ1][ISIZ2][5][5],
                  double FIXP_ANN ldx[ISIZ1][ISIZ2][5][5],
                  double FIXP_ANN d[ISIZ1][ISIZ2][5][5],
                  int ist, int iend, int jst, int jend,
                  int nx0, int ny0 ) __attribute__((always_inline)) {
/*--------------------------------------------------------------------
c
...
--------------------------------------------------------------------*/
  int i, j, m;
  double FIXP_ANN tmp, tmp1;
  double FIXP_ANN tmat[5][5];
...
--------------------------------------------------------------------*/
                 double FIXP_ANN v[ISIZ1][ISIZ2/2*2+1][ISIZ3/2*2+1][5],
                 double FIXP_ANN tv[ISIZ1][ISIZ2][5],
                 double FIXP_ANN d[ISIZ1][ISIZ2][5][5],
                 double FIXP_ANN udx[ISIZ1][ISIZ2][5][5],
                 double FIXP_ANN udy[ISIZ1][ISIZ2][5][5],
                 double FIXP_ANN udz[ISIZ1][ISIZ2][5][5],
                 int ist, int iend, int jst, int jend,
                 int nx0, int ny0 ) __attribute__((always_inline)) {
/*--------------------------------------------------------------------
c
...
--------------------------------------------------------------------*/
  ...
  int ist1, iend1;
  int jst1, jend1;
  double FIXP_ANN dsspm;
  double FIXP_ANN xi, eta, zeta;
  double FIXP_ANN q;
  double FIXP_ANN u21, u31, u41;
  double FIXP_ANN tmp;
  double FIXP_ANN u21i, u31i, u41i, u51i;
  double FIXP_ANN u21j, u31j, u41j, u51j;
  double FIXP_ANN u21k, u31k, u41k, u51k;
  double FIXP_ANN u21im1, u31im1, u41im1, u51im1;
  double FIXP_ANN u21jm1, u31jm1, u41jm1, u51jm1;
  double FIXP_ANN u21km1, u31km1, u41km1, u51km1;
...
static void exact( int i, int j, int k, double FIXP_ANN u000ijk[5] ) __attribute__((always_inline)) {
...
--------------------------------------------------------------------*/
  int i, j;
  double FIXP_ANN r43;
  double FIXP_ANN c1345;
  double FIXP_ANN c34;
  double FIXP_ANN tmp1, tmp2, tmp3;
...
--------------------------------------------------------------------*/
                    double FIXP_ANN v[ISIZ1][ISIZ2/2*2+1][ISIZ3/2*2+1][5],
                    double FIXP_ANN sum[5]) __attribute__((always_inline)) {
...
--------------------------------------------------------------------*/
  int i, j, k, m;
  double FIXP_ANN sum0=0.0, sum1=0.0, sum2=0.0, sum3=0.0, sum4=0.0;
...
--------------------------------------------------------------------*/
  ...
  if (fp != NULL) {
    ...
    for (int i=0; i<5; i++) tolrsd[i] = vtmp[i];
    while(fgetc(fp) != '\n');
...
--------------------------------------------------------------------*/
  ...
  int iglob, jglob;
  double FIXP_ANN xi, eta, zeta;
  double FIXP_ANN pxi, peta, pzeta;
  double FIXP_ANN ue_1jk[5],ue_nx0jk[5],ue_i1k[5],
...
--------------------------------------------------------------------*/
  ...
  int istep;
  double FIXP_ANN tmp;
  double FIXP_ANN delunm[5], tv[ISIZ1][ISIZ2][5];
