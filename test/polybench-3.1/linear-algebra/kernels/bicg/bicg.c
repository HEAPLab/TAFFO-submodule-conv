...
void init_array (int nx, int ny,
		 ...
		 DATA_TYPE POLYBENCH_1D(p,NY,ny)) __attribute__((always_inline))
{
  int i, j;

  for (i = 0; i < ny; i++)
    p[i] = i * M_PI / (ny * sqrt(ny));
  for (i = 0; i < nx; i++) {
    r[i] = i * M_PI / (nx * sqrt(nx));
    for (j = 0; j < ny; j++)
      A[i][j] = ((DATA_TYPE) i*(j+1)) / (nx * ny * sqrt(nx * ny));
  }
}
...
void print_array(int nx, int ny,
		 ...
		 DATA_TYPE POLYBENCH_1D(q,NX,nx)) __attribute__((always_inline))
...
void kernel_bicg(int nx, int ny,
		 ...
		 DATA_TYPE POLYBENCH_1D(r,NX,nx)) __attribute__((always_inline))
{
  int i, j;
