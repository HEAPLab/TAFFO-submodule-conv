...
void init_array(int ni, int nj,
		...
		DATA_TYPE POLYBENCH_2D(B,NI,NJ,ni,nj)) __attribute__((always_inline))
{
  int i, j;
  ...
  *beta = 21.23;
  for (i = 0; i < ni; i++)
    for (j = 0; j < nj; j++) {
      ...
      B[i][j] = ((DATA_TYPE) i*j) / (ni*nj*sqrt(ni*nj));
    }
  for (i = 0; i < ni; i++)
    for (j = 0; j < ni; j++)
...
void print_array(int ni,
		 DATA_TYPE POLYBENCH_2D(C,NI,NI,ni,ni)) __attribute__((always_inline))
{
  int i, j;
