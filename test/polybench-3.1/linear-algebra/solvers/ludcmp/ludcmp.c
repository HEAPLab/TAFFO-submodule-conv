...
void init_array (int n,
		 ...
		 DATA_TYPE POLYBENCH_1D(y,N+1,n+1)) __attribute__((always_inline))
{
  int i, j;
  ...
  for (i = 0; i <= n; i++)
    {
      ...
      b[i] = (i+1)/n + 42;
      for (j = 0; j <= n; j++) {
	A[i][j] = i==j ? ((DATA_TYPE) (i+1)*(j+1)) / n : 0.0 ;
      }
    }
}
...
void print_array(int n,
		 DATA_TYPE POLYBENCH_1D(x,N+1,n+1)) __attribute__((always_inline))
