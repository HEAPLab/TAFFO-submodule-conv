...
void init_array(int length,
		...
		DATA_TYPE POLYBENCH_2D(W,LENGTH,LENGTH,length,length)) __attribute__((always_inline))
{
  int i, j;
  for (i = 0; i < length; i++)
    for (j = 0; j < length; j++) {
      c[i][j] = (i*j % 2) ;
      W[i][j] = ((DATA_TYPE) i-(j/49)) / length;
    }
}
...
static
void print_array(DATA_TYPE out) __attribute__((always_inline))
{
...
void kernel_dynprog(int tsteps, int length,
		    ...
		    DATA_TYPE *out) __attribute__((always_inline))
{
  int iter, i, j, k;
