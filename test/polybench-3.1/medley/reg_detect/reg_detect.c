...
void init_array(int maxgrid,
		...
		DATA_TYPE POLYBENCH_2D(path,MAXGRID,MAXGRID,maxgrid,maxgrid)) __attribute__((always_inline))
{
  int i, j;
...
void kernel_reg_detect(int niter, int maxgrid, int length,
		       ...
		       DATA_TYPE POLYBENCH_3D(sum_diff,MAXGRID,MAXGRID,LENGTH,maxgrid,maxgrid,length)) __attribute__((always_inline))
{
  int t, i, j, cnt;
