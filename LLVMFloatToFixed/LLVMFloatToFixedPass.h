...
namespace flttofix {
    ...
    struct FloatToFixed : public llvm::ModulePass {
        ...
        void
        readGlobalMetadata(llvm::Module &m, llvm::SmallPtrSetImpl<llvm::Value *> &res, bool functionAnnotation = false);

        void
        readLocalMetadata(llvm::Function &f, llvm::SmallPtrSetImpl<llvm::Value *> &res, bool onlyArguments = false);

        void readAllLocalMetadata(llvm::Module &m, llvm::SmallPtrSetImpl<llvm::Value *> &res);

        bool
        parseMetaData(llvm::SmallPtrSetImpl<llvm::Value *> *variables, mdutils::MDInfo *fpInfo, llvm::Value *instr);
        ...
        llvm::Constant *
        convertGlobalVariable(llvm::GlobalVariable *glob, FixedPointType &fixpt, TypeMatchPolicy typepol);

        llvm::Constant *convertConstantExpr(llvm::ConstantExpr *cexp, FixedPointType &fixpt, TypeMatchPolicy typepol);

        llvm::Constant *
        convertConstantAggregate(llvm::ConstantAggregate *cag, FixedPointType &fixpt, TypeMatchPolicy typepol);

        llvm::Constant *convertConstantDataSequential(llvm::ConstantDataSequential *, const FixedPointType &);

        template<class T>
        llvm::Constant *createConstantDataSequential(llvm::ConstantDataSequential *, const FixedPointType &);

        llvm::Constant *
        convertLiteral(llvm::ConstantFP *flt, llvm::Instruction *, FixedPointType &, TypeMatchPolicy typepol);
        ...
        llvm::Value *
        translateOrMatchOperandAndType(llvm::Value *val, const FixedPointType &fixpt, llvm::Instruction *ip = nullptr) {
        ...
        llvm::Value *
        genConvertFloatToFix(llvm::Value *flt, const FixedPointType &fixpt, llvm::Instruction *ip = nullptr);
         ...
         *    is an instruction or a constant.
         *  @returns The converted value. */
        llvm::Value *genConvertFixedToFixed(llvm::Value *fix, const FixedPointType &srct, const FixedPointType &destt,
                                            llvm::Instruction *ip = nullptr);
        ...
        llvm::Type *
        getLLVMFixedPointTypeForFloatType(llvm::Type *ftype, const FixedPointType &baset, bool *hasfloats = nullptr);
            ...
            if (Instruction *from = dyn_cast<Instruction>(src)) {
                ...
                if (mdargs) {
                    Constant *mdtid = cast<ConstantAsMetadata>(
                            mdargs->getOperand(arg->getArgNo() * 2).get())->getValue();
