...
Value *FloatToFixed::convertCall(CallSite *call, FixedPointType &fixpt) {
    ...
    if (!newF) {
        LLVM_DEBUG(dbgs() << "[Info] no function clone for instruction" << *(call->getInstruction())
                          << ", engaging fallback\n");
        return Unsupported;
    }
...
Value *FloatToFixed::convertBinOp(Instruction *instr, const FixedPointType &fixpt) {
    ...
    if (opc == Instruction::FAdd || opc == Instruction::FSub || opc == Instruction::FRem) {
        ...
        if(fixpt.isFixedPoint()) {
            ...
            updateFPTypeMetadata(fixop, intermtype.scalarIsSigned(), intermtype.scalarFracBitsAmt(),
        ...
        if(fixpt.isFixedPoint()) {
            ...
            cpMetaData(ext2, val2);
            cpMetaData(fixop, instr);
            updateFPTypeMetadata(fixop, fixoptype.scalarIsSigned(), fixoptype.scalarFracBitsAmt(),
