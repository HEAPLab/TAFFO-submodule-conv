...
static RegisterPass<FloatToFixed>
    X("flttofix", "Floating Point to Fixed Point conversion pass",
      true /* Does not only look at CFG */, true /* Optimization Pass */);
...
void FloatToFixed::openPhiLoop(PHINode *phi) {
  ...
  if (phi->materialized_use_empty()) {
    LLVM_DEBUG(
        dbgs()
        << "phi" << *phi
        << " not currently used by anything; skipping placeholder creation\n");
    return;
  }

  info.placeh_noconv =
      createPlaceholder(phi->getType(), phi->getParent(), "phi_noconv");
  *(newValueInfo(info.placeh_noconv)) = *(valueInfo(phi));
  ...
  if (isFloatingPointToConvert(phi)) {
    Type *convt =
        getLLVMFixedPointTypeForFloatType(phi->getType(), fixPType(phi));
  ...
  LLVM_DEBUG(dbgs() << "created placeholder (non-converted=["
                    << *info.placeh_noconv << "], converted=["
...
void FloatToFixed::closePhiLoops() {
  ...
  for (auto data : phiReplacementData) {
    ...
    if (!substphi) {
      LLVM_DEBUG(
          dbgs() << "phi " << *origphi
                 << "could not be converted! Trying last resort conversion\n");
    ...
    LLVM_DEBUG(dbgs() << "restored data flow of original phi " << *origphi
                      << " to new value " << *substphi << "\n");
  }
...
void FloatToFixed::sortQueue(std::vector<Value *> &vals) {
  ...
  while (next < vals.size()) {
    ...
    for (auto *u : v->users()) {
      ...
      if (!hasInfo(u)) {
        LLVM_DEBUG(
            dbgs() << "[WARNING] Value " << *u
                   << " will not be converted because it has no metadata\n");
  ...
  for (Value *v : vals) {
    assert(hasInfo(v) &&
           "all values in the queue should have a valueInfo by now");
    if (fixPType(v).isInvalid() &&
        !(v->getType()->isVoidTy() && !isa<ReturnInst>(v))) {
      LLVM_DEBUG(
          dbgs()
          << "[WARNING] Value " << *v
  ...
  for (Value *qi : q) {
    ...
    if (cqi == ConversionError) {
      ...
      LLVM_DEBUG(qi->print(errs());
                 if (Instruction *i = dyn_cast<Instruction>(qi)) errs()
  ...
  auto clear = [&](bool (*toDelete)(const Instruction &Y)) {
    for (Value *v : q) {
      ...
      if (operandPool[v] == v) {
        LLVM_DEBUG(
            dbgs() << *i
                   << " not deleted, as it was converted by self-mutation\n");
        continue;
      }
      ...
      for (Value *root : roots) {
        if (!isrootok[root]) {
          LLVM_DEBUG(i->print(errs()); errs() << " not deleted: involves root ";
                     root->print(errs()); errs() << '\n');
          allok = false;
          break;
        }
      }
...
void FloatToFixed::propagateCall(std::vector<Value *> &vals,
  ...
  for (int i = 0; i < vals.size(); i++) {
    ...
    if (!newF) {
      LLVM_DEBUG(dbgs() << "Attempted to clone function " << oldF->getName()
                        << " but failed\n");
      continue;
    }
    ...
    LLVM_DEBUG(dbgs() << "Converting function " << oldF->getName() << " : "
                      << *oldF->getType() << " into " << newF->getName()
                      << " : " << *newF->getType() << "\n");

    ValueToValueMapTy
        origValToCloned; // Create Val2Val mapping and clone function
    ...
    if (!specialFunction)
      for (int i = 0; oldIt != oldF->arg_end(); oldIt++, newIt++, i++) {
        if (oldIt->getType() != newIt->getType()) {
          ...
          Value *placehValue =
              createPlaceholder(oldIt->getType(), &newF->getEntryBlock(), name);
          /* Reimplement RAUW to defeat the same-type check (which is ironic
           * because we are attempting to fix a type mismatch here) */
...
Function *FloatToFixed::createFixFun(CallSite *call, bool *old, bool *special) {
  ...
  if (!oldF->getMetadata(SOURCE_FUN_METADATA)) {
    LLVM_DEBUG(dbgs() << "createFixFun: function " << oldF->getName()
                      << " not a clone; ignoring\n");
    return nullptr;
  }

  std::vector<Type *> typeArgs;
  std::vector<std::pair<int, FixedPointType>>
      fixArgs; // for match already converted function
  ...
  for (auto arg = oldF->arg_begin(); arg != oldF->arg_end(); arg++, i++) {
    ...
    if (hasInfo(v)) {
      fixArgs.push_back(
          std::pair<int, FixedPointType>(i, valueInfo(v)->fixpType));
  ...
  if (newF) {
    LLVM_DEBUG(dbgs() << *(call->getInstruction())
                      << " use already converted function : " << newF->getName()
                      << " " << *newF->getType() << "\n";);
    if (old)
      *old = true;
    return newF;
  ...
  LLVM_DEBUG({
    dbgs() << "creating function " << oldF->getName() << "_" << suffix
           << " with types ";
  ...
  newF = Function::Create(newFunTy, oldF->getLinkage(),
                          oldF->getName() + "_" + suffix, oldF->getParent());
...
void FloatToFixed::printConversionQueue(std::vector<Value *> vals) {
  if (vals.size() > 1000) {
    dbgs()
        << "not printing the conversion queue because it exceeds 1000 items\n";
    return;
  }
