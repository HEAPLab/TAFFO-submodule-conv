...
TaffoMath::createFixedPointFromConst(
  ...
  TaffoMath::pair_ftp_value<llvm::Constant *, TaffoMath::TABLELENGHT>    arctan_2power;
    ...
    generic = builder.CreateICmpSLT(builder.CreateLoad(i), ConstantInt::get(Type::getInt32Ty(cont), arg_size/2));
    builder.CreateCondBr(generic, cordic_body, end_loop );

    builder.SetInsertPoint(cordic_body);

    // y>0
    generic= builder.CreateICmpSGE(builder.CreateLoad(y.value),  zero);
    
    // d = 1 if x >= t else -1
    builder.CreateStore(builder.CreateSelect(
        builder.CreateICmpSGE(builder.CreateLoad(x.value), builder.CreateLoad(t.value)), 
    ...
    generic = builder.CreateShl(builder.CreateLoad(builder.CreateGEP(arctan_g, {zero,builder.CreateLoad(i)})), ConstantInt::get(Type::getInt32Ty(cont), 1));
    generic = builder.CreateSelect(builder.CreateICmpEQ(builder.CreateLoad(d.value), zero),builder.CreateSub(zero, generic), generic);
    builder.CreateStore(builder.CreateAdd(builder.CreateLoad(theta.value), generic ), theta.value);


    //    t = t + t*math.pow(2,-2*i) 
    generic = builder.CreateShl(builder.CreateLoad(i), ConstantInt::get(Type::getInt32Ty(cont), 1));
    builder.CreateStore(
        builder.CreateAdd(
            builder.CreateLoad(t.value), 
            builder.CreateAShr( builder.CreateLoad(t.value), generic
            )), t.value);


    //    x1= x-d*math.pow(2,-i)*y
    generic = builder.CreateAShr(builder.CreateLoad(y.value), builder.CreateLoad(i));
    builder.CreateStore(
        builder.CreateSelect(
            builder.CreateICmpEQ(builder.CreateLoad(d.value), zero), 
                builder.CreateAdd(builder.CreateLoad(x.value), generic), 
                builder.CreateSub(builder.CreateLoad(x.value), generic)), 
        x1.value);

    
    // y1= d*math.pow(2,-i)*x + y
    generic = builder.CreateAShr(builder.CreateLoad(x.value), builder.CreateLoad(i));
    builder.CreateStore(
        builder.CreateSelect(
            builder.CreateICmpEQ(builder.CreateLoad(d.value), zero), 
                builder.CreateSub(builder.CreateLoad(y.value), generic), 
                builder.CreateAdd(builder.CreateLoad(y.value), generic)), 
        y1.value);


    //    x= x1-d*math.pow(2,-i)*y1
    generic = builder.CreateAShr(builder.CreateLoad(y1.value), builder.CreateLoad(i));
    builder.CreateStore(
        builder.CreateSelect(
            builder.CreateICmpEQ(builder.CreateLoad(d.value), zero), 
                builder.CreateAdd(builder.CreateLoad(x1.value), generic), 
                builder.CreateSub(builder.CreateLoad(x1.value), generic)), 
        x.value);
    
    // y= d*math.pow(2,-i)*x1 + y1
    generic = builder.CreateAShr(builder.CreateLoad(x1.value), builder.CreateLoad(i));
    builder.CreateStore(
        builder.CreateSelect(
            builder.CreateICmpEQ(builder.CreateLoad(d.value), zero), 
                builder.CreateSub(builder.CreateLoad(y1.value), generic), 
                builder.CreateAdd(builder.CreateLoad(y1.value),generic)), 
        y.value);

    builder.CreateStore(builder.CreateAdd(builder.CreateLoad(i), ConstantInt::get(Type::getInt32Ty(cont), 1)), i );
    ...
    if(fxpret.scalarFracBitsAmt() > arg_size - 3){
        builder.CreateStore(builder.CreateAShr(builder.CreateLoad(theta.value), fxpret.scalarFracBitsAmt() - arg_size + 3), theta.value);
    }else if (fxpret.scalarFracBitsAmt() < arg_size - 3){
        builder.CreateStore(builder.CreateShl(builder.CreateLoad(theta.value), -fxpret.scalarFracBitsAmt() + arg_size - 3), theta.value);
    }
    builder.CreateRet(builder.CreateLoad(theta.value));
...
TaffoMath::createFixedPointFromConst(
  ...
  TaffoMath::pair_ftp_value<llvm::Constant *, TaffoMath::TABLELENGHT>    arctan_2power;
    ...
    // x>0
    generic= builder.CreateICmpSGE(builder.CreateLoad(x.value),  zero);
    
    // d = 1 if x >= t else -1
    builder.CreateStore(builder.CreateSelect(
        builder.CreateICmpSGE(builder.CreateLoad(t.value), builder.CreateLoad(y.value)), 
        builder.CreateSelect(generic, one.value, zero),         
        builder.CreateSelect(generic, zero, one.value))
    ,d.value);
